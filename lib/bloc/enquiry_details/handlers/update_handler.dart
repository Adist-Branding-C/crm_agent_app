...
extension UpdateHandler on EnquiryDetailsBloc {
  Future<void> onUpdateDetails(UpdateEnquiryDetails ev, Emitter<EnquiryDetailsState> emit) async {
    final s = state;
    if (s is! EnquiryDetailsLoaded) return;
    final updated = Lead(
      id: s.lead.id, name: ev.name, phone: ev.phone, email: ev.email,
      location: ev.location, leadSource: ev.source, source: ev.purpose,
      category: ev.category, status: ev.status,
      nextFollowUp: ev.nextFollowUp, note: ev.note,
    );
    ...
    final newAct = EnquiryActivity(
      ...
      time: 'Just now', type: EnquiryActivityType.system,
    );
