...
* **Styling:**
    ...
    * File length: Every source code file (coding pages) must be strictly under 80 lines of code. Split large classes, widgets, and styles into multiple smaller files using modular exports or sub-widgets.
...
* **Library Decomposition (Part/Part Of):** For cohesive structures or large libraries (such as a BLoC consisting of Bloc, Event, and State), split them across multiple files using the `part` and `part of` directives. The primary file acts as the library header, importing external packages and declaring parts using `part 'filename.dart';`. The sub-files must declare `part of 'primary_filename.dart';` and must not include independent import statements.
...
* **Refactoring Build Methods:** When refactoring large build methods, extract
  distinct UI components strictly into separate `StatelessWidget` classes. Avoid
  using `StatefulWidget`; if state handling is required, implement BLoC for
  managing the business logic and state uniformly.
